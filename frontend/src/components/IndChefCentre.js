...
function ccyFormat(nums) {
  let frais = Boolean(nums.length) && nums.reduce((s, a) => s + a.indemnite, 0);
...
export default function IndChefCentre({
  download,
  setDownload,
  session,
  region,
  ...
  React.useEffect(() => {
    protectedApi
      .get(
        `/sessions/${session.id}/sessioncentres?type=indccs&region=${region?.id}`
      )
      .then((res) => {
  ...
  React.useEffect(() => {
    if (download) {
      protectedApi
        .get(
          `/sessions/${session.id}/sessioncentres?type=indccs&format=csv&region=${region?.id}`,
          {
            responseType: "blob",
          }
        )
        .then((res) => {
          ...
          link.href = url;
          link.setAttribute("download", "indemnitesChefCentre.csv");
          link.click();
          setDownload(false);
