...
function ccyFormatNbVaccC(nums) {
  let salles =
    Boolean(nums.length) &&
    nums.reduce((s, a) => s + a.nbr_vaccation_correct_cm, 0);
  return Boolean(nums.length) ? `${salles.toLocaleString("fr-Fr")}` : "0";
}
...
function ccyFormatTotalVacc(nums) {
  let survs =
    Boolean(nums.length) && nums.reduce((s, a) => s + a.montant_vacc_cm, 0);
...
function ccyFormatIndemnite(nums) {
  let survss =
    Boolean(nums.length) && nums.reduce((s, a) => s + a.indemnite_cm, 0);
  return Boolean(nums.length)
...
function ccyFormat(nums) {
  let frais = Boolean(nums.length) && nums.reduce((s, a) => s + a.total_cm, 0);
  return Boolean(nums.length)
...
export default function IndCm({ download, setDownload, session, region }) {
    ...
    if (download) {
      protectedApi
        ...
        .then((res) => {
          ...
          link.href = url;
          link.setAttribute("download", "vaccations_charge_mission_ind.csv");
          link.click();
          setDownload(false);
